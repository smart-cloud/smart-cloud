...
class MetricsOkTest {
    ...
    void testMetrics() throws InterruptedException {
        ...
        List<Instance> instances = instanceRegistry.getInstances().filter(Instance::isRegistered).filter(instance -> {
        ...
        for (Instance instance : instances) {
            instanceMetricsMonitorComponents.forEach(instanceMetricsMonitorComponent -> {
                try {
                    Assertions.assertThat(instanceMetricsMonitorComponent.alert(instance)).isEqualTo(false);
                } catch (IOException e) {
