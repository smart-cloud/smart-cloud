 ...
 */
@Slf4j
public class LiveThreadCountMonitorComponent extends AbstractInstanceMetricsMonitorComponent<Integer> {
    ...
    public MetricCheckResultDTO check(Instance instance) throws IOException {
        ...
        try {
            ...
            int alertThreshold = monitorProperties.getMetric().getLiveThreadCountThreshold();
            // 触发阈值
            ...
            // 连续新增
            if (matchKeepIncreasing(name, currentLiveThreadCount)) {
                String alertDesc = String.format("活动线程数连续新增超过预警值[%d次]，当前线程数[%f]", monitorProperties.getMetric().getKeepIncreasingCount(), currentLiveThreadCount);
    ...
    public long getCheckIntervalSeconds() {
        return monitorProperties.getMetric().getThreadCheckIntervalSeconds();
    }
