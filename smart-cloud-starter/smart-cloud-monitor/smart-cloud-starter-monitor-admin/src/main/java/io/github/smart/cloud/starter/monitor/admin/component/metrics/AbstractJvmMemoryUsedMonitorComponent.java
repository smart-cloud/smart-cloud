 ...
 */
@Slf4j
public abstract class AbstractJvmMemoryUsedMonitorComponent extends AbstractInstanceMetricsMonitorComponent<Long> {

    /**
     * 获取内存预警值
     *
     * @param monitorProperties
     * @param instanceName      服务实例名
     * @return
     */
    protected abstract DataSize getAlertThreshold(MonitorProperties monitorProperties, String instanceName);
    ...
    public MetricCheckResultDTO check(Instance instance) throws IOException {
        ...
        try {
            ...
            DataSize alertThreshold = getAlertThreshold(monitorProperties, name);
            // 触发阈值
            if (currentSize.compareTo(alertThreshold) >= 0) {
                String alertDesc = String.format("当前值[%dMB]超过预警值[%dMB]", currentSize.toMegabytes(), alertThreshold.toMegabytes());
                return MetricCheckResultDTO.error(MetricCheckStatus.THRESHOLD_EXCEPTION, alertDesc);
            }

            // 连续新增
            if (matchKeepIncreasing(instance.getId().toString(), valueNode.asLong())) {
                String alertDesc = String.format("内存连续新增超过预警值[%d次]，当前内值[%dMB]，有内存泄漏倾向", monitorProperties.getMetric().getKeepIncreasingCount(), currentSize.toMegabytes());
